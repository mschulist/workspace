...
class intro(Slide):
    def construct(self):
        ...
        problem = Text("Imagine you are exploring an asteroid belt, but you can only see the adjacent asteroids.", font_size=24).to_edge(UP)
        generic_asteroids = ImageMobject("generic_belt.png").scale(0.35).to_edge(DOWN)
        self.add(problem, generic_asteroids)
        self.play(Write(problem), FadeIn(generic_asteroids))
        self.wait()
        self.next_slide()

        # self.play(problem.animate.to_edge(UP))
        specific_problem = Text("The number of days you spend on each asteroid must be proportional to the relative size of the asteroid.", font_size=24).next_to(problem, DOWN)
        ...
        question = Text("We need to develop a method to explore our asteroids\nwithout being able to see all of them at once.", font_size=32).next_to(specific_problem, DOWN)
        self.add(question)
        self.play(Write(question))
        self.wait()
        self.next_slide()

        self.play(Unwrite(problem), Unwrite(specific_problem), Unwrite(question), FadeOut(generic_asteroids))
...
class explanation(Slide):
    def construct(self):
        ...
        reason3 = Text("3. Power of Computers", font_size=36).next_to(reason2, DOWN).to_edge(LEFT)
        self.add(reason3)
        ...
        self.remove(reason1, reason2, reason3, useful)
        self.play(FadeOut(reason1), FadeOut(reason2), FadeOut(reason3), FadeOut(useful))
        self.wait()
        self.next_slide()
        ...
        bbwo = ImageMobject("bbwo.png").scale(0.25).to_edge(UP).to_edge(LEFT)
        bbwo_description = Text("Black-backed Woodpecker", font_size=26).next_to(bbwo, DOWN)
        self.add(bbwo, bbwo_description)
        self.play(FadeIn(bbwo), Write(bbwo_description))
        self.wait()
        self.next_slide()

        data_collection = Text("Data Collected Through\nHuman Point Counts and\nAutonomous Recording Units", font_size=36).next_to(bbwo, RIGHT)
        self.add(data_collection)
        self.play(Write(data_collection))
        self.wait()
        self.next_slide()

        traceplot = ImageMobject("beta1_traceplot.png").scale(0.4).next_to(bbwo_description, DOWN).set_x(0)
        self.add(traceplot)
        self.play(FadeIn(traceplot))
        self.wait()
        self.next_slide()

        self.play(Unwrite(data_collection), FadeOut(bbwo), Unwrite(bbwo_description))
        self.play(traceplot.animate.to_edge(UP))
        bbwo_distribution = ImageMobject("bbwo_dist.png").scale(0.69).next_to(traceplot, DOWN).to_edge(LEFT)
        self.add(bbwo_distribution)
        self.play(FadeIn(bbwo_distribution))
        self.wait()
        self.next_slide()

        bbwo_regression = ImageMobject("bbwo_beta1_regres.png").scale(0.45).next_to(bbwo_distribution, RIGHT).to_edge(RIGHT)
        self.add(bbwo_regression)
        self.play(FadeIn(bbwo_regression))
        self.wait()
        self.next_slide()

        sidenote = Text("With one year of data,\nBlack-backed Woodpecker\npopulations decreased with an\nincrease in burn severity", font_size=24).next_to(bbwo_regression, LEFT)
        self.add(sidenote)
        self.play(Write(sidenote))
        self.wait()
        self.next_slide()

        self.play(FadeOut(bbwo_distribution), FadeOut(bbwo_regression), Unwrite(sidenote), FadeOut(traceplot))
        ...
        people = Text("Jack Dumbacher (California Academy of Sciences)\nDurrell Kapan (California Academy of Sciences)\nTom Denton (Google Research)\nLauren Harrell (Google Research)\nMatt Harvey (Google Research)\nKevin Rees", font_size=36).next_to(thanks, DOWN)
        self.add(people)
