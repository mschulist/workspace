...
class intro(Slide):
    def construct(self):
        ...
        rule1 = (Text("Step 1: Start at a random asteroid", font_size=24).to_edge(UP))
        self.next_slide()

        # Add the image generated by DALL-E here
        self.next_slide()

        self.play(Unwrite(rule1))
        rule2 = Text("Step 2: Flip a fair coin p(Heads) = 0.5 to decide\nwhether to propose going clockwise or counterclockwise.", font_size=24).to_edge(UP)
        self.next_slide()

        coinCircle = Circle(radius=0.5, color=RED, fill_opacity=1).next_to(rule2, DOWN)
        Heads = Text("H", font_size=36).move_to(coinCircle.center())
        self.add(coinCircle, Heads)
        self.play(Create(coinCircle, Heads))
        self.next_slide()

        clockwise = Text("Propose to move clockwise").next_to(coinCircle, DOWN)
        self.add(clockwise)
        self.play(Write(clockwise))
        self.next_slide()

        rule3 = MathTex("\\text{Step 3: Flip a weighted coin, with p(heads)} = \\dfrac{\\text{proposal}}{\\text{current}}", font_size=24).next_to(rule2, DOWN).to_edge(LEFT)
        self.add(rule3)
