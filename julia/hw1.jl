...
function vecvec_to_matrix(vecvec)
	nvecs = size(vecvec, 1)
	ncols = size(vecvec[1], 1)
	m = zeros(Int, nvecs, ncols)
	for i in 1:nvecs
		m[i, :] = vecvec[i]
	end
	return m
...
function matrix_to_vecvec(matrix)
	nrow = size(matrix, 1)
	ncol = size(matrix, 2)
	vecvec = Vector(undef, nrow)
	for i in 1:nrow
		vecvec[i] = view(matrix, i, :)
	end	
	return vecvec
end
...
begin
	...
	function quantize(color::AbstractRGB)
		r = floor(color.r * 10) / 10
		g = floor(color.g * 10) / 10
		b = floor(color.b * 10) / 10
		return RGB(r,g,b)
	end
	
	function quantize(image::AbstractMatrix)
		h,w = size(image)
		quantized = Matrix(undef, h, w)
		for i in 1:h, j in 1:w
			p = image[i,j]
			quantized[i,j] = quantize(p)
		end
		return RGB.(quantized)
	end
end
...
# ╔═╡ b463f77a-6a98-4bb7-ae17-197a8ce3a1e2
red.r

# ╔═╡ 846b1330-ee0b-11ea-3579-7d90fafd7290
md"Can you invert the picture of Philip?"

# ╔═╡ c043a9ab-1d97-4d67-84df-caf036c11da8
begin
	function invert(image::AbstractMatrix)
		h,w = size(image)
		inverted = Matrix(undef, h, w)
		for i in 1:h, j in 1:w
...
begin
	...
	function noisify(image::AbstractMatrix, s)
		h,w = size(image)
		noisy = Matrix(undef, h, w)
		for i in 1:h, j in 1:w
			p = image[i,j]
			noisy[i,j] = noisify(p,s)
			end
		return RGB.(noisy)
	end
end
...
# ╔═╡ b5b7d4ea-5d12-443a-b2bc-2a29aa71cd51
philip
...
function blur_1D(v, l)
	vlen = size(v, 1)
	lhalf = (l-1) ÷ 2
	vext = [extend(v, i) for i in 1-lhalf:vlen+lhalf]
	vextlen = size(vext,1)
	blurred = Vector{Float64}(undef, vlen)
...
# ╔═╡ 749b065c-7457-4724-b6d7-97103fc660a3
@bind l_box Slider(1:9, show_value=true)
...
function convolve_vector(v, k)
	l = (length(k) - 1) ÷ 2
	vext = [extend(v, i) for i in 1-l:length(v)+l]
	blurred = Vector(undef, length(v))
	for i in 1:length(v)
		sum = 0
		for j in 1:length(k)
			sum += vext[i+j-1] * k[j]
...
# ╔═╡ 11141fba-e74b-41b5-bc73-937a1d77520b
display("2")
...
# ╔═╡ 76ecb37e-c5e3-4f32-ac9f-ffd8c9b5d22d
convolve_vector([1,2,3,4,5], [0,1,0])
...
function gaussian_kernel(n)
	l = (n + 1) ÷ 2
	g(x) = (1/sqrt(2*π)) * exp((-x^2)/2)
	k = [g(i) for i in 1-l:n-l]
	s = sum(k)
	return [k[i]/s for i in 1:length(k)]
end
...
gaussian_kernel_size_1D = 9 # change this value, or turn me into a slider!

# ╔═╡ a23f1811-d8c2-4ddd-aaac-07bc7276c0a3
gaussian_kernel(9)
...
# ╔═╡ 999240d3-3518-4e71-bf31-dde5c21cea9a
...
# ╔═╡ eb413361-4fdf-4869-8be8-6bd9d24a3254
webcam
